...
//         /* free memory inside the union, if needed! */
//         /* For example, if you dynamically allocated the numbers array in .data or the string in .string */
//         if (current->type == DIRECTIVE_STATEMENT)
//         {
//             if (current->content.directive.type == DATA)
//             {
//                 if (current->content.directive.params.data.values != NULL)
//                 {
//                     free(current->content.directive.params.data.values);
//                 }
//             }
//             else if (current->content.directive.type == STRING)
//             {
//                 if (current->content.directive.params.str != NULL)
//                     free(current->content.directive.params.str);
//             }

//             /* For .entry and .extern, the label is managed by the Symbol Table, or it was not dynamically allocated within this union */
//         }
//         else if (current->type == INSTRUCTION_STATEMENT)
//         {
//             if (current->content.instruction.opcode != NULL)
//                 free(current->content.instruction.opcode);
